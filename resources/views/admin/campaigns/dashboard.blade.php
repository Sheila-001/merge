...
=======
        <a href="{{ route('admin.campaigns.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg inline-flex items-center space-x-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <h2 class="text-2xl font-bold text-gray-900">{{ $campaigns->count() }}</h2>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <h5 class="text-gray-500 text-sm font-medium mb-2">Total Donations</h5>
            <h2 class="text-2xl font-bold text-gray-900">{{ $campaigns->sum('donations_count') }}</h2>
        </div>
        <div class="bg-white rounded-lg shadow-md p-6">
            <h5 class="text-gray-500 text-sm font-medium mb-2">Total Amount Raised</h5>
            <h2 class="text-2xl font-bold text-gray-900">₱{{ number_format($campaigns->sum('donations_sum_amount'), 2) }}</h2>
        </div>
    </div>
...
=======
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $campaign->status === 'Ongoing' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $campaign->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $campaign->donations_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">₱{{ number_format($campaign->current_amount, 2) }}</td>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $donation->campaign_title }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">₱{{ number_format($donation->amount, 2) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $donation->status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $donation->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $donation->created_at->format('M d, Y H:i') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
